...
const service = {
  ...
  tutoringSessions: () => FIND_MANY_RESULT,
  tutoringSession: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("TutoringSession", () => {
  ...
  test("POST /tutoringSessions", async () => {
    await request(app.getHttpServer())
      .post("/tutoringSessions")
      .send(CREATE_INPUT)
  ...
  test("GET /tutoringSessions", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /tutoringSessions/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tutoringSessions"}/${nonExistingId}`)
  ...
  test("GET /tutoringSessions/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tutoringSessions"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /tutoringSessions existing resource", async () => {
